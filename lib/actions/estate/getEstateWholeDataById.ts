...
export async function getEstateWholeDataById(id: number) {
  ...
  const translation = await db.query.estateTranslation.findMany({
    where: (table) => eq(table.estateId, id),
  });
...
export type EstateData = Awaited<ReturnType<typeof getEstateWholeDataById>>;
